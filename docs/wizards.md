...
5. **Monitor state.** `Prompt.AskMonitorState` → `monitor: enabled | disabled |
   ...
   storage check with `mounted: true` and `free_pct: { op: "<", value: 5 }`, so
   the same unit reports a missing mount and critically low free space. They are operated by
...
7. **Wizard-specific options.** For watches: thresholds (`volume`), metrics
  ...
  mounts: use the fixed 5% low-space threshold and ask only mount-specific
  safety options such as whether Sermo should use refcounting; unmount
  force/lazy/kill choices are made per CLI/Web action.
