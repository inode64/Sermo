...
Sermo configuration is split by document kind: **daemons/apps/libs/patterns**
in the catalog, **services** as concrete monitored instances, and **mounts** as
fstab-backed mount units. Include directories may also contain global fragments
with a top-level `watches:` or `notifiers:` map; those fragments do not use
`kind:`.
...
/etc/sermo/notifiers/*.yml included notifier fragments
/etc/sermo/mounts/*.yml   fstab-backed mount documents
/etc/sermo/<watch-type>/*.yml     wizard-generated watch fragments
...
paths:
  ...
  includes:
    - /etc/sermo/services
    - /etc/sermo/apps            # legacy alias; keep only while old service files live there
    - /etc/sermo/notifiers       # optional: one notifier fragment per file
    - /etc/sermo/storage         # optional: generated by `sermoctl wizard volume`
    - /etc/sermo/network         # optional: generated by `sermoctl wizard net`
  mounts:
    - /etc/sermo/mounts
  runtime: /run/sermo
  state: /var/lib/sermo
...
new service, notifier or watch fragment under these directories should be
...
Notifier fragments may live under any directory listed in `paths.includes`
(commonly `/etc/sermo/notifiers`). Each file contains exactly one notifier under
the top-level `notifiers:` map:
...
Include directories may contain service documents, notifier fragments or watch
fragments. A watch fragment is a normal YAML file with a top-level `watches:`
map and exactly one watch:

```yaml
# /etc/sermo/storage/storage-root.yml
watches:
  storage-root:
    monitor: previous
...
same one-entry rule under a top-level `notifiers:` map.
