...
sections. Enabled directories may also contain watch fragments with a top-level
...
> watch per file. The wizard adds that directory to `paths.enabled` in
...
Enabled directories may contain either service documents or watch fragments. A
