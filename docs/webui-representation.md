...
| Events | `GET /api/events` | cursor page of service/watch activity; supports `limit`, `since`, `service`, `watch`, `kind`, `status`, `only_errors`; the dashboard's range selector defaults to the last 24 hours, with "any time" an explicit choice |
...
`engine.default_timeout` only for a check that declares none. The HTTP write
deadline is raised to that same budget so the panel receives the result.
...
| Process metrics | the same CPU, memory and IO panels a service detail shows for its own runtime — one titled box each, with its avg/min/max summary and its chart — on a 1h/24h/7d/30d/1y window |
