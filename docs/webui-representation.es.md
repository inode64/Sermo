...
| Expansión de servicio | `GET /api/services/{name}` | checks, información del proceso y reglas; cada métrica graficable de un check lleva su último `value` numérico fresco cuando existe, distinguiendo un cero medido de una muestra ausente o caducada |
...
| Watches | `GET /api/watches` | watches de host y de service; `scope` los distingue y los nombres de watch de service usan `service:watch`; cada métrica graficable lleva su último `value` numérico fresco cuando existe, para que la expansión del servicio pueda mostrarlo sin volver a sondear |
...
| Eventos | `GET /api/events` | página por cursor de actividad de servicios/watches; admite `limit`, `since`, `service`, `watch`, `kind`, `status`, `only_errors`; el selector de rango del panel arranca en las últimas 24 horas, con "any time" como elección explícita |
...
| Área | Contenido |
| --- | --- |
| Datos generales | una cuadrícula sin encabezado, primera área de la expansión: nombre, estado, categoría, unidad/backend, uptime, intervalo, política, locks, último evento, próxima remediación, estado de remediación, totales del proceso y los últimos valores frescos de checks graficables y watches de ámbito servicio; las muestras ausentes o caducadas se omiten en lugar de mostrarse como cero; mientras la insignia de la fila sea `starting`, la expansión puede mostrar todavía el backend de init en bruto (`inactive`) y muestras de check en curso del ciclo de solo observación |
| Gráficos | línea temporal de SLA a ancho completo seguida de gráficos de latencia, métricas numéricas de checks y watches de ámbito servicio, CPU, memoria e IO; cada panel sigue la ventana persistida del servicio y reutiliza la serie que pertenece a su check/watch; los servicios `no_resident_process` omiten los gráficos de runtime de procesos, pero conservan SLA y las métricas configuradas de checks/watches |
...
Los valores actuales graficables de checks/watches son la excepción deliberada
y acotada a la regla de no repetición: permanecen en la cuadrícula Datos
generales para una lectura operativa rápida, mientras su serie histórica sigue
en Gráficos y el diagnóstico completo permanece en Checks o en el panel Watches.
Solo aparecen muestras frescas publicadas por el daemon, y un cero medido se
muestra como cero en vez de confundirse con datos ausentes.
...
renderizan con sus estados distintos, y un servidor tmux presente y vacío con
socket configurado se renderiza con una píldora roja `empty` y un `close` que
mata el servidor por la API tras confirmación. Un origen disponible sin nadie
conectado — el de ssh ante todo — no dice nada y no renderiza fila; aparece en
