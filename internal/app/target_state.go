...
// set rather than started/stopped. An outage outranks an advisory: a watch whose
// error counter warns while its link is down reads failed, not warning.
func WatchState(enabled, monitored, failed, warning, observed bool) string {
