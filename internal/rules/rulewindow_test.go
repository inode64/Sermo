...
func TestFormatCondition(t *testing.T) {
	cases := []struct {
		...
		{"metric unknown field", map[string]any{"metric": map[string]any{"metric": "memory"}}, "metric"},
		{"service", map[string]any{"service": map[string]any{"state": "active"}}, "service:active"},
		{"process exe", map[string]any{"process": map[string]any{"exe": "/usr/bin/worker", "user": "app"}}, "process:/usr/bin/worker"},
		{"process user fallback", map[string]any{"process": map[string]any{"user": "app"}}, "process:app"},
		...
		{"changed app", map[string]any{"changed": map[string]any{"app": "containerd"}}, "changed:containerd"},
