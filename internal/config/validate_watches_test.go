...
func TestValidateNotifiers(t *testing.T) {
	...
	for _, w := range []string{
		...
		"notifiers.bad-type.type \"smoke-signal\" is not supported (email, slack, teams)",
...
func TestValidateWatchesBad(t *testing.T) {
	cases := map[string]map[string]any{
		...
		"for mode":               {"check": map[string]any{"type": "storage", "path": "/", "used_pct": map[string]any{"op": ">=", "value": 90}}, "then": map[string]any{"hook": map[string]any{"command": []any{"/x"}}}, "for": map[string]any{"cycles": 3, "mode": "consecutive"}},
		...
		"within mode":            {"check": map[string]any{"type": "storage", "path": "/", "used_pct": map[string]any{"op": ">=", "value": 90}}, "then": map[string]any{"hook": map[string]any{"command": []any{"/x"}}}, "within": map[string]any{"cycles": 5, "min_matches": 2, "mode": "within"}},
