...
func TestSLATimelinesBucketsSegmentsIntoSubSpans(t *testing.T) {
	...
	tls, err := s.SLATimelines(slaTestService, now)
	if err != nil {
...
func TestCheckSLATimelinesScopeToCheck(t *testing.T) {
	...
	if err := s.RecordCheckSLA(slaTestService, "http", true, now.Add(-3*time.Minute)); err != nil {
	...
	tls, err := s.CheckSLATimelines(slaTestService, "http", now)
	if err != nil {
	...
	ghost, err := s.CheckSLATimelines(slaTestService, "ghost", now)
	if err != nil {
