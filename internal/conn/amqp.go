...
func (amqpProtocol) Probe(ctx context.Context, cfg Config) (Result, error) {
	c, err := probeTargetFor(ctx, cfg, defaultPortAMQP).openStream(ctx)
	if err != nil {
		return Result{}, err
	}
	...
	if _, err := c.Write(amqpHeader); err != nil {
		return Result{}, probeErr(ProtocolNameAMQP, stepAMQPProtocolHeader, err)
	}

	// A method frame header is type(1), channel(2), size(4).
	var hdr [amqpFrameHeaderSize]byte
	if _, err := io.ReadFull(c, hdr[:]); err != nil {
		return Result{}, probeErr(ProtocolNameAMQP, stepAMQPFrameHeader, err)
	}
	...
	if _, err := io.ReadFull(c, payload); err != nil {
		return Result{}, probeErr(ProtocolNameAMQP, stepAMQPFramePayload, err)
	}
