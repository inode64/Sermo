...
func TestInspectUsesConfiguredUserLookupForOwners(t *testing.T) {
	...
	runner := commandRunner(map[string]execx.Result{
	...
	report := inspectResolved(context.Background(), fakeRunner(nil), "app", config.Resolved{Name: "app", Tree: tree(path, nil)}, config.CategoryApp, WithUserLookup(lookup))
...
func TestInspectVersionCommandOutcomes(t *testing.T) {
	...
	ok := inspect(t, fakeRunner(map[string]execx.Result{
		bin: {Stdout: "\nTool v2.8.4.1-extra\n", ExitCode: 0},
	...
	stderrVer := inspect(t, fakeRunner(map[string]execx.Result{
		bin: {Stderr: "Tool 1.2\n", ExitCode: 0},
	}), tree(bin, version(nil)))
	...
	wrongExit := inspect(t, fakeRunner(map[string]execx.Result{
		bin: {Stderr: "boom\n", ExitCode: 2},
	}), tree(bin, version(nil)))
	...
	expectedExit := inspect(t, fakeRunner(map[string]execx.Result{
		bin: {Stdout: "Tool 3.0\n", ExitCode: 3},
	...
	expectedExitList := inspect(t, fakeRunner(map[string]execx.Result{
		bin: {Stdout: "Tool 3.1\n", ExitCode: 1},
	...
	mosquittoHelp := inspect(t, fakeRunner(map[string]execx.Result{
	...
	badStdout := inspect(t, fakeRunner(map[string]execx.Result{
		bin: {Stdout: "garbage\n", ExitCode: 0},
	...
	failed := inspect(t, runnerWithErr(map[string]execx.Result{bin: {ExitCode: 0}}, errors.New("exec format error")), tree(bin, version(nil)))
	if failed.OK || !strings.Contains(failed.Status, "exec format error") {
		t.Errorf("runner error: %+v", failed)
	}

	optionalFailure := inspect(t, fakeRunner(map[string]execx.Result{
...
func TestInspectHealthCommandTakesPriority(t *testing.T) {
	...
	r := inspect(t, commandRunner(map[string]execx.Result{
...
func TestListFiltersMissingBinaries(t *testing.T) {
	...
	only := List(context.Background(), fakeRunner(nil), cfg, config.CategoryApp, false)
	if len(only) != 1 || only[0].Name != "present" {
		t.Fatalf("installed-only = %+v, want [present]", only)
	}
	all := List(context.Background(), fakeRunner(nil), cfg, config.CategoryApp, true)
	if len(all) != 2 {
		t.Fatalf("includeMissing = %+v, want 2 reports", all)
	}
	if List(context.Background(), fakeRunner(nil), nil, config.CategoryApp, true) != nil {
...
func setupVersionMatchCatalog(t *testing.T, versionByBin map[string]string) (*config.Config, *execxtest.Runner, string) {
	t.Helper()
	root := t.TempDir()
	...
	for name, output := range versionByBin {
		runner.ByLine[commandKey(filepath.Join(binDir, name), "--version")] = execx.Result{Stdout: output, ExitCode: 0}
	}
...
`, local, local),
	...
	if got := runner.Count(rpcmountd, "-v"); got != 1 {
...
`, filepath.Join(binDir, "php${version}")),
	...
	reports := List(context.Background(), fakeRunner(nil), cfg, config.CategoryApp, false)
	var names []string
	for _, report := range reports {
		names = append(names, report.Name)
...
// fakeRunner answers by binary path; commands without an entry fail like a
// missing binary would.
func fakeRunner(byPath map[string]execx.Result) *execxtest.Runner {
	return &execxtest.Runner{ByName: byPath, Default: execx.Result{ExitCode: 127, Stderr: "not found"}}
}

// commandRunner answers by full command line (see commandKey); any other
// command fails as not found.
func commandRunner(byCommand map[string]execx.Result) *execxtest.Runner {
	return &execxtest.Runner{ByLine: byCommand, Default: execx.Result{ExitCode: 127, Stderr: "not found"}, Err: errors.New("not found")}
}

func commandKey(name string, args ...string) string {
	return execxtest.Line(name, args...)
}

// runnerWithErr answers like fakeRunner but every configured binary also
// reports err, as a broken executable would.
func runnerWithErr(byPath map[string]execx.Result, err error) *execxtest.Runner {
	r := fakeRunner(byPath)
	r.Errs = make(map[string]error, len(byPath))
