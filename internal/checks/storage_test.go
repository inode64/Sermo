...
func TestStorageCheckInodesUsedPct(t *testing.T) {
	...
	breach := storageCheck{name: "d", path: "/", preds: []levelPred{{"inodes_used_pct", ">=", 90}}, usage: fakeStorageStats(stats)}
	...
	ok := storageCheck{name: "d", path: "/", preds: []levelPred{{"inodes_free", "<", 1000}}, usage: fakeStorageStats(stats)}
...
func TestStorageCheckInodesUnavailableNeverFires(t *testing.T) {
	...
	c := storageCheck{name: "d", path: "/", preds: []levelPred{{"inodes_free", "<", 1000}}, usage: fakeStorageStats(stats)}
	if c.Run(context.Background()).OK {
...
func TestStorageCheckDataHasValueKey(t *testing.T) {
	...
	c := storageCheck{name: "d", path: "/", preds: []levelPred{{"used_pct", ">=", 90}}, usage: fakeStorage(92, 8, 80, 1000)}
	...
	c2 := storageCheck{name: "d", path: "/", preds: []levelPred{{"free_pct", "<", 5}}, usage: fakeStorage(96, 4, 40, 1000)}
	if v := c2.Run(context.Background()).Data["value"]; v != 4.0 {
		t.Fatalf("value = %v, want 4.0 (free_pct)", v)
	}
}

func TestStorageCheckPreservesZeroUsedBytes(t *testing.T) {
	stats := StorageStats{TotalBytes: 100, FreeBytes: 70}
	c := storageCheck{name: "storage", path: "/data", preds: []levelPred{{field: fieldUsedBytes, op: "==", value: 0}}, usage: fakeStorageStats(stats)}
	res := c.Run(context.Background())
