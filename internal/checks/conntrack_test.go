...
func TestConntrackUsedPct(t *testing.T) {
	...
	breach := conntrackCheck{base: base{name: "c"}, preds: []levelPred{{"used_pct", ">=", 90}}, sampler: fakeConntrack(sample)}
	if res := breach.Run(context.Background()); !res.OK {
		t.Fatalf("92%% should breach >= 90, got %q", res.Message)
	}
	ok := conntrackCheck{base: base{name: "c"}, preds: []levelPred{{"used_pct", ">=", 95}}, sampler: fakeConntrack(sample)}
...
func TestConntrackFreeAbsolute(t *testing.T) {
	c := conntrackCheck{base: base{name: "c"}, preds: []levelPred{{"free", "<", 10000}}, sampler: fakeConntrack(ConntrackSample{Count: 95000, Max: 100000})}
