...
func TestLoadThreshold(t *testing.T) {
	...
	breach := loadCheck{base: base{name: "l"}, preds: []loadPred{{"load5", ">", 5}}, sampler: fakeLoad(sample)}
	if res := breach.Run(context.Background()); !res.OK {
		t.Fatalf("load5 7 should breach > 5, got %q", res.Message)
	}
	ok := loadCheck{base: base{name: "l"}, preds: []loadPred{{"load5", ">", 10}}, sampler: fakeLoad(sample)}
...
func TestLoadPerCPUNormalizes(t *testing.T) {
	...
	perCPU := loadCheck{base: base{name: "l"}, perCPU: true, preds: []loadPred{{"load5", ">", 1.0}}, sampler: fakeLoad(sample)}
	...
	over := loadCheck{base: base{name: "l"}, perCPU: true, preds: []loadPred{{"load1", ">", 1.0}}, sampler: fakeLoad(sample)}
...
func TestLoadPerCPUNoCPUFails(t *testing.T) {
	c := loadCheck{base: base{name: "l"}, perCPU: true, preds: []loadPred{{"load1", ">", 1}}, sampler: fakeLoad(LoadSample{Load1: 9, NumCPU: 0})}
...
func TestLoadMultiPredAnd(t *testing.T) {
	...
	c := loadCheck{base: base{name: "l"}, preds: []loadPred{{"load5", ">", 5}, {"load15", ">", 5}}, sampler: fakeLoad(sample)}
	if c.Run(context.Background()).OK {
