...
func TestCLIRejectsMalformedCommands(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "daemon missing subcommand", args: []string{"daemon"}, want: "daemon requires a subcommand"},
		{name: "daemon bad subcommand", args: []string{"daemon", "nope"}, want: `unknown daemon subcommand "nope"`},
		{name: "daemon list extra", args: []string{"daemon", "list", "extra"}, want: "daemon list takes no arguments"},
		{name: "daemon show missing", args: []string{"daemon", "show"}, want: "daemon show requires exactly one daemon name"},
		{name: "daemon show extra", args: []string{"daemon", "show", "web", "extra"}, want: "daemon show requires exactly one daemon name"},
		{name: "service missing subcommand", args: []string{"service"}, want: "service requires a subcommand"},
		{name: "service bad subcommand", args: []string{"service", "nope"}, want: `unknown service subcommand "nope"`},
		{name: "service list extra", args: []string{"service", "list", "extra"}, want: "service list takes no arguments"},
		{name: "service show missing", args: []string{"service", "show"}, want: "service show requires exactly one service name"},
		{name: "service show extra", args: []string{"service", "show", "web", "extra"}, want: "service show requires exactly one service name"},
		{name: "service clone missing target", args: []string{"service", "clone", "web"}, want: "service clone requires SOURCE and TARGET"},
		{name: "service clone extra", args: []string{"service", "clone", "web", "copy", "extra"}, want: "service clone requires SOURCE and TARGET"},
