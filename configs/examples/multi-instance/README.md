# Multiple instances from one profile
...
[`uses` + `variables`](../../../docs/profiles.md) inheritance. The profile
parametrizes everything that varies with `${...}` placeholders; each enabled
service `uses` the profile and overrides only the variables that make it unique.

Files:

- `profiles/dbserver.yml` — the shared profile. Note the `config` variable and
...
Use `uses` when N instances should all derive from the *profile*: each instance
