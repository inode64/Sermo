#!/usr/bin/env bash
# Local orchestrator: install Sermo on remote hosts that do not have it yet.
#
# Companion to update_fleet.sh, which only refreshes already configured hosts.
# Per host it uploads the locally built payload and runs remote_stage.sh
# (binaries, packaged catalog, a minimal /etc/sermo/sermo.yml and read-only
# inventory), regenerates that host's configuration locally with
# generate_install_config.py, then applies it with remote_apply.sh, which
# enables and starts sermod through the host's real init.
#
# Unreachable or unhealthy hosts are recorded and skipped, never forced through
# (see README.md "Fleet install and update failure handling").
set -u
...
usage() {
	cat <<'USAGE'
usage: install_fleet.sh [options] HOST [HOST...]

Install Sermo on remote hosts that do not have it yet: stage the payload,
regenerate each host's configuration locally, apply it and enable sermod.
...
environment:
  SERMO_WEB_PASSWORD          admin password for the generated configuration and
                              Web UI verification (default: sermo-remote-admin)
  SERMO_SSH_OPTS              extra options for every ssh/scp invocation
  SERMO_READY_WAIT_SECONDS    forwarded to the remote scripts

A host that already has /etc/sermo is reinstalled: remote_stage.sh overwrites
sermo.yml and remote_apply.sh replaces the generated directories. Back up
/etc/sermo first when the host carries manual tweaks worth keeping.
USAGE
}

die() {
	echo "install_fleet: $*" >&2
	exit 64
}
...
if [ "$dry_run" = "1" ]; then
	...
	echo "  3. run remote_stage.sh ${run_id} ${remote_dir}/${payload_name}"
	echo "  4. fetch the staged inventory"
	echo "  5. regenerate config locally (generate_install_config.py)"
	echo "  6. apply config with remote_apply.sh ${run_id} (enables sermod)"
	echo "hosts:"
	printf '  %s\n' "${hosts[@]}"
	exit 0
fi
...
for host in "${hosts[@]}"; do
	...
	if ! run_ssh "$host" "env SERMO_WEB_PASSWORD='${web_password}' SERMO_READY_WAIT_SECONDS='${SERMO_READY_WAIT_SECONDS:-240}' bash ${remote_dir}/remote_stage.sh ${run_id} ${remote_dir}/${payload_name}"; then
		echo "  stage failed; collecting artifacts and skipping" >&2
		record "$host" "stage" "failed" "remote_stage.sh non-zero"
		fetch_failure_artifacts "$host" "/tmp/sermo-install-${run_id}/out.tar.gz"
		failures=$((failures + 1))
		continue
	fi
	record "$host" "stage" "ok" "binaries staged; inventory collected"

	stage_root="${run_root}/stage/${host}"
	mkdir -p "${stage_root}/${host}"
	if ! run_scp "${ssh_user}@${host}:/tmp/sermo-install-${run_id}/out.tar.gz" "${host_dir}/inventory.tar.gz" \
		|| ! tar -C "${stage_root}/${host}" -xzf "${host_dir}/inventory.tar.gz"; then
		echo "  inventory download failed; config not generated" >&2
	...
	if [ ! -f "$config_tgz" ]; then
		echo "  generator produced no config for ${host}" >&2
	...
	if ! run_scp "$config_tgz" "${ssh_user}@${host}:${remote_dir}/sermo-config.tgz" \
		|| ! run_ssh "$host" "env SERMO_WEB_PASSWORD='${web_password}' SERMO_READY_WAIT_SECONDS='${SERMO_READY_WAIT_SECONDS:-240}' bash ${remote_dir}/remote_apply.sh ${run_id} ${remote_dir}/sermo-config.tgz"; then
		echo "  config apply failed" >&2
		record "$host" "apply" "failed" "remote_apply.sh non-zero"
		fetch_failure_artifacts "$host" "/tmp/sermo-apply-${run_id}/out.tar.gz"
		failures=$((failures + 1))
		continue
	fi
	record "$host" "apply" "ok" "fresh install; sermod enabled and active"

	run_ssh "$host" "rm -rf ${remote_dir} /tmp/sermo-install-${run_id} /tmp/sermo-apply-${run_id}" || true
	echo "  ok"
done
